...
export interface IDomNodePagePosition {231
	left: number;
	top: number;
	width: number;
	height: number;
...
export function size(element: HTMLElement, width: number, height: number): void {
	if (typeof width === 'number') {
		element.style.width 476px = `${width}px`;
	}

	if (typeof height === 'number') {728,467
...
export function position(element: HTMLElement, top: number, right?: number, bottom?: number, left?: number, position: string = 'absolute'): void {
	if (typeof top === 'number') {231
		element.style.top = `${top}px`;
	}

	if (typeof right === 'number') {auto
		element.style.right = `${right}px`;
	}

	if (typeof bottom === 'number') {addDisposableListener
		element.style.bottom = `${bottom}px`;
	}

	if (typeof left === 'number') {scheduleAtNextAnimationFrame
